...
class MainActivity : AppCompatActivity(), NavigationView.OnNavigationItemSelectedListener,
    ...
    // 측정소 이름을 파라미터로 하여 미세먼지 정보 불러오기
    private fun stationFineDust(stationName: String, callback: (pm10: String, pm25: String) -> Unit) {
    ...
    // 측정소 이름으로 측정소 위치 받아오기
    private suspend fun fetchStationCoordinates(stationName: String, sidoName: String): LatLng? {
    ...
    override fun onNavigationItemSelected(item: MenuItem): Boolean {
        when(item.itemId){
            R.id.item1 -> {Log.d("mobileApp", "네비게이션 뷰 메뉴 1")} // 지워도되나
            R.id.item2 -> {
            ...
            R.id.item3 -> {Log.d("mobileApp", "네비게이션 뷰 메뉴 3")} // 지워도되나
            R.id.item4 -> {
            ...
            .addOnSuccessListener { location: Location? ->
                if (location != null) {
                    ...
                    naverMap.locationOverlay.run {
                        isVisible = true // 위치 오버레이의 가시성은 기본적으로 false로 지정되어 있습니다. 가시성을 true로 변경하면 지도에 위치 오버레이가 나타납니다.
